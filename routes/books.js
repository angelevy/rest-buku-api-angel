...
router.post('/', upload.single('image'), (req, res) => {
    ...
    const imageUrl = `http://192.168.1.4:3000/uploads/${file.filename}`;
...
router.put('/:id', upload.single('image'), (req, res) => {
    ...
    if (file && books[index].image) {
        const oldImagePath = books[index].image.replace('http://192.168.1.4:3000/', ''); // Sesuaikan IP kamu
        if (fs.existsSync(oldImagePath)) fs.unlinkSync(oldImagePath);
        books[index].image = `http://192.168.1.4:3000/uploads/${file.filename}`;
    }
...
router.delete('/:id', (req, res) => {
    ...
    const imagePath = books[index].image.replace('http://192.168.1.4:3000/', ''); // Sesuaikan IP kamu
